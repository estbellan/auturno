  ...
  async findOrCreateFromAuthClaims(claims: AuthClaims): Promise<CurrentUserContext> {
    ...
    if (!user.workshopId) {
      const invitedCustomer = await this.customerModel
        .findOne({ email, inviteStatus: 'invited', authSubject: null })
        .exec();
      ...
      if (invitedCustomer) {
        const updated = await this.userModel
          .findByIdAndUpdate(
            user._id,
            { workshopId: invitedCustomer.workshopId, roles: ['client'], permissions: [] },
            { new: true },
          )
          .exec();

        if (updated) {
          return this.toCurrentUserContext(updated);
        }
      }
    }
